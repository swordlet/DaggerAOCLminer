...
#define rot(x, y) (((u)x << (u)y) | ((u)x >> ((u)32U-(u)y)))
#define Ch(x, y, z) bitselect((u)z, (u)y, (u)x)

#define ITERATIONS_COUNT 8192*512
...
channel uint8 chan_sha __attribute__((depth(8192)));
channel uint8 chan_output __attribute__((depth(8192)));

__attribute__((max_global_work_dim(0)))
__kernel void search_nonce(ulong startNonce,
    __constant uint* restrict preCalcState,
    __constant uint* restrict data)
{
    ...
    {        
        ...
        write_channel_intel(chan_sha, ch);

    }
}

__attribute__((max_global_work_dim(0)))
__kernel void sha_hash(__constant uint* restrict state)
{
    u t1, t2;
    for(uint i = 0; i < ITERATIONS_COUNT; ++i)
    {        
        u a, b, c, d, e, f, g, h; // data from channel uint8
        uint8 from_chan;
        from_chan = read_channel_intel(chan_sha);
        a = from_chan.s0;
        b = from_chan.s1;
        c = from_chan.s2;
        d = from_chan.s3;
        ...
        ch.s6 = g2 + g;
        ch.s7 = h2 + h;
        write_channel_intel(chan_output, ch);
    }
}


__attribute__((max_global_work_dim(0)))
__kernel void nonce_output(ulong startNonce,  
...
{  
    ...
    for(uint i = 0; i < ITERATIONS_COUNT; ++i)
    {
        u t1, t2;
        uint8 from_chan;
        from_chan = read_channel_intel(chan_output);
        u w0 = from_chan.s0;
        u w1 = from_chan.s1;
        u w2 = from_chan.s2;
        u w3 = from_chan.s3;
    ...
    {
        uint slot = min((uint)OUTPUT_SIZE, (uint)atomic_inc((__global volatile uint*)output) + 1);
