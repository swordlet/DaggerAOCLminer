...
void MinerManager::Execute()
{
    if(_shouldListDevices)
    {
        ...
        {
            AOCLMiner::ListDevices(_useOpenClCpu);
        }
...
void MinerManager::StreamHelp(ostream& _out)
{
    _out
        ...
        << "    -cl-local-work Set the OpenCL local work size. Default is " << AOCLMiner::_defaultLocalWorkSize << endl
        << "    -cl-global-work Set the OpenCL global work size as a multiple of the local work size. Default is " << AOCLMiner::_defaultGlobalWorkSizeMultiplier << " * " << AOCLMiner::_defaultLocalWorkSize << endl
...
void MinerManager::DoBenchmark(MinerType type, unsigned warmupDuration, unsigned trialDuration, unsigned trials)
{
    ...
    FillRandomTask(taskProcessor.GetNextTask()); // generate random data for benchmark
    taskProcessor.SwitchTask();

    Farm farm(&taskProcessor, _io_service);
    farm.AddSeeker(Farm::SeekerDescriptor { &AOCLMiner::Instances, [](unsigned index, XTaskProcessor* taskProcessor) { return new AOCLMiner(index, taskProcessor); } });
...
void MinerManager::ConfigureGpu()
{
    ...
    {
        AOCLMiner::SetDevices(_openclDevices, _openclDeviceCount);
        _openclMiningDevices = _openclDeviceCount;
    }

    if(!AOCLMiner::ConfigureGPU(
        _localWorkSize,
    ...
    AOCLMiner::SetNumInstances(_openclMiningDevices);
    AOCLMiner::SetUseNvidiaFix(_useNvidiaFix, _nvidiaSpinDamp);
    AOCLMiner::SetUseVectors(_useVectors);
}
